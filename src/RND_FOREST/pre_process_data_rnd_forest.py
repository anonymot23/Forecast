...
class AirDataPreProcessorRndForest(AirDataPreProcessor):
    ...
    def save_air(self, data: pd.DataFrame) -> None:
        ...
        """
        filename_air_save = filename_air_processed_rnd_forest()
        filepath = join(DATA_FOLDER, filename_air_save)
        data.to_csv(filepath)        
        
        
def filename_air_processed_rnd_forest(suff: str = "") -> str:
    """
    ...
    """
    return f"AirPassengersProcessedRndForest_{suff}.csv"
...
if __name__ == "__main__":
    ...
    shift = 1
    lag = 12
    air_preprocessor = AirDataPreProcessorRndForest(shift, lag)
